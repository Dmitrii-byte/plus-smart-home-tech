...
public class ScenarioAnalyzer {
    ...
    public void analyzeSnapshot(SensorsSnapshotAvro snapshot) {
        ...
        log.info("=== SNAPSHOT ANALYSIS ===");
        log.info("Hub: {}", hubId);
        log.info("Number of sensors: {}", sensorsState.size());

        List<Scenario> scenarios = scenarioRepository.findByHubId(hubId);
        log.info("Found scenarios for hub {}: {}", hubId, scenarios.size());

        if (scenarios.isEmpty()) {
            log.warn("No scenarios for hub {}", hubId);
            return;
        }
        ...
        for (Scenario scenario : scenarios) {
            ...
            if (checkScenarioConditions(scenario, sensorsState)) {
                ...
                log.info("✗ Scenario '{}' not activated for hub {}", scenario.getName(), hubId);
            }
        }
    }
    ...
    private void executeAction(Action action, String hubId, String scenarioName) {
        try {
            log.info("Sending action: hubId={}, scenario={}, sensorId={}, type={}, value={}",
